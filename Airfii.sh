...
# Author: Abhishek Rai - nmap y pa' dentro
...
function ctrl_c(){
	echo -e "\n${yellowColour}[*]${endColour}${grayColour}Coming out${endColour}"
...
function helpPanel(){
	echo -e "\n${yellowColour}[*]${endColour}${grayColour} Use: ./Airfii.sh${endColour}"
...
function dependencies(){
	...
	echo -e "${yellowColour}[*]${endColour}${grayColour} Checking necessary programs...${endColour}"
	sleep 2
	...
	for program in "${dependencies[@]}"; do
		...
		if [ "$(echo $?)" == "0" ]; then
			...
			echo -e "${yellowColour}[*]${endColour}${grayColour} installing tool ${endColour}${blueColour}$program${endColour}${yellowColour}...${endColour}"
...
function startAttack(){
	...
	if [ "$(echo $attack_mode)" == "Handshake" ]; then
		...
		echo -e "\n${redColour}[*] This attack mode is invalid${endColour}\n"
	fi
}
...
if [ "$(id -u)" == "0" ]; then
	...
	echo -e "\n${redColour}[*] i am not root${endColour}\n"
fi
